...
func (d *AwardsPacket) GetID() int64 {
	return d.ID
}

// SetID sets ID to the packet.
func (d *AwardsPacket) SetID(id int64) {
	d.ID = id
}

// GetSend returns whether to send this packet.
func (d *AwardsPacket) GetSend() bool {
	return d.Send
}

// SetSend sets whether to send this packet.
func (d *AwardsPacket) SetSend(value bool) {
	d.Send = value
}
...
func (packet *AwardsPacket) Read(b buffer.PacketBuffer) {
	...
	if packet.AwardID != 0 {
		...
		for i, _ := range packet.Awards {
			...
			for j, _ := range packet.Awards[i].Level {
	...
	for i, _ := range packet.PlayerFavAwards {
